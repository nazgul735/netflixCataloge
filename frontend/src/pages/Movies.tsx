...
import {MOVIE} from "../api/graphqlQueries";
...
function Movies() {
  ...
  const {data: movieData} = useQuery<GetMoviesQueryType>(MOVIE, {variables:{"limit": 10, "offset": offset}});
  const limit=10; 
  console.log(movieData);
  const handleChange = (event: React.ChangeEvent<any>, page: number) => {
    setOffset(limit*(page-1)+1);
  };
  ...
  return (
    <div className="grid-container">
        ...
        <Pagination className="pagination" count={movieData?.getMovies?.pages || 1} onChange={handleChange}/>
