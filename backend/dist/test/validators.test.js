...
test('Confirming that errors of illegal username has value', () => {
    const { errors, valid } = validators_1.validateLoginInput(" ", "");
    expect(`${errors.username}`).toEqual('Username needs value');
    expect(`${valid}`).toMatch(/[false|null]/);
});
test('Confirming legal values is valid', () => {
    const { errors, valid } = validators_1.validateLoginInput("test", "test");
    expect(`${valid}`).toMatch(/[true]/);
});
//validateRegisterInput
test('Invalid register values', () => {
    const { errors, valid } = validators_1.validateRegisterInput("", "", "password", "passwordNotEqual");
...
test('Valid register values', () => {
    const { errors, valid } = validators_1.validateRegisterInput("username", "email@email.com", "password", "password");
    expect(`${valid}`).toMatch(/[true]/);
});
