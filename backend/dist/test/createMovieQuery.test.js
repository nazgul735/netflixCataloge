...
describe("Correctly creates title query", () => {
    jest_each_1.default(genTestCases(100, getTitleQuery, () => randomString(25))).test("", (input, expected, i) => {
        let created = createMovieQuery_1.createMovieQuery(input, null, null, null);
        expect(created).toEqual(expected);
    });
});
describe("Correctly creates genre query", () => {
    jest_each_1.default(genTestCases(100, getGenreQuery, () => randomString(25))).test("", (input, expected, i) => {
        let created = createMovieQuery_1.createMovieQuery(null, input, null, null);
        expect(created).toEqual(expected);
    });
});
describe("Correctly creates from and to year query", () => {
    jest_each_1.default(genTestCases(100, getYearQuery, () => [randomInt(9999), randomInt(9999)])).test("", ([from, to], expected, i) => {
        let created = createMovieQuery_1.createMovieQuery(null, null, from, to);
        expect(created).toEqual(expected);
    });
});
describe("Correctly creates arbitrary query", () => {
    jest_each_1.default(genTestCases(100, getCompleteQuery, () => [randomInt(9999), randomInt(9999)])).test("", ([title, genre, from, to], expected, i) => {
        let created = createMovieQuery_1.createMovieQuery(title, genre, from, to);
        expect(created).toEqual(expected);
    });
});
